...
NTL.phys.data.TuesdayLake.1990to1999 <- filter(NTL.phys.data, NTL.phys.data$lakename == 'Tuesday Lake' & NTL.phys.data$year4 >= '1990' & NTL.phys.data$year4 <= '1999') 

```
Question: Why don't we filter using row numbers?

> Answer: Bc it's silly, row numbers can change as you move data around. Filtering by attributes is smarter and more fullproof
...
Piped.NTL.phys.data.TuesdayLake.1990to1999.July <- 
  NTL.phys.data %>% 
  filter(lakename == 'Tuesday Lake' & year4 >= '1990' & year4 <= '1999') %>% 
...
NTL.phys.data.TuesdayLake.1990to1999.meantemp <- 
  Piped.NTL.phys.data.TuesdayLake.1990to1999.July %>% 
  filter(depth == 0) %>% 
...
NTL.phys.data.collapsedirradiance <- 
  pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = 'Irradiance_Type',
