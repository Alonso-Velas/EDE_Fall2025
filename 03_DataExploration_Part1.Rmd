...
> Answer: tidyverse packages are downloaded. There are conflicts with dplyr::filter() and dplyr::lag()
...
> ANSWER: In metadata, it's from USGS, Eno River in NC!
...
#always comment out the view command before knitting
...
> Answer: Finding means with missing values does not work, so you can omit it first!
