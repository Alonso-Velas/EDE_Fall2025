...
```{r project.setup, message=FALSE, warning=FALSE, error=FALSE, include=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
\newpage
```{r boxplot1, echo=FALSE, fig.align='left', fig.cap='Monthly Ozone Concentrations (2018) Boxplot', fig.height=3, fig.width=7, warning=FALSE}
boxplot1 <- epa_data %>% 
  filter(Year==2018) %>% 
  ggplot(aes(x = factor(Month,levels=1:12), y = Ozone)) +
  geom_boxplot() +
  scale_x_discrete(labels = month.abb) +
  labs(x = "Month", y = "Ozone (µg/m³)", title = "Monthly Ozone Concentrations (2018)")
boxplot1
```

```{r boxplot2, echo=FALSE, fig.align='left', fig.cap='Monthly Ozone Concentrations (2019) Boxplot', fig.height=3, fig.width=7, warning=FALSE}
boxplot2 <- epa_data %>% 
  filter(Year==2019) %>% 
  ggplot(aes(x = factor(Month,levels=1:12), y = Ozone)) +
  geom_boxplot() +
  scale_x_discrete(labels = month.abb) +
  labs(x = "Month", y = "Ozone (µg/m³)", title = "Monthly Ozone Concentrations (2019)")
boxplot2
```
...
## Git Repository
[Click here for Alonso's Github Repository Link](https://github.com/Alonso-Velas/EDE_Fall2025)

I did not use AI in the completion of this assignment.

Signed, Alonso Velasquez
October 20, 2025
