...
LTER_Lake_ChemistryPhysics_df <- read.csv(here('./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv'),
                                          stringsAsFactors = TRUE)
view(LTER_Lake_ChemistryPhysics_df)
# 2.
library(tidyverse);library(here)

#you need the full.names = TRUE to copy the full path
filenames <- dir(here('Data','Raw'),pattern = ("EPAair_O3*.*"), full.names = TRUE)
print(filenames)
EPAair_O3_df <- filenames %>%
  map(read_csv) %>% 
  reduce(rbind)
#yay!! Okay so full.names = TRUE is important when reading the files from the directory
#map(read_csv gets )
